...
(function () {
    ...
    const ENCODED_PASSWORD = btoa('Su^f8##wT%TN');
    const PASSWORD_KEY = 'pixverse_script_auth';
    const EXPIRY_KEY = 'pixverse_script_expiry';
    ...
    function checkPassword() {
        ...
        if (!storedPassword || storedPassword !== ENCODED_PASSWORD || !expiry || Date.now() > parseInt(expiry)) {
            ...
            if (encodedInput === ENCODED_PASSWORD) {
                localStorage.setItem(PASSWORD_KEY, encodedInput);
                localStorage.setItem(EXPIRY_KEY, Date.now() + EXPIRY_TIME);
                alert('รหัสผ่านถูกต้อง สคริปต์เริ่มทำงาน');
            } else {
